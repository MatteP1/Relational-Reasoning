...
\begin{proof}
  ...
  \end{align}
  it suffices to show $\LogRel{\tenv}{\venv}{\Let \varB = \expr_1 in \fun \var . \expr_2}{\fun \var . \Let \varB = \expr_1 in \expr_2}{\Tfunc{\typ_1}{\typ_2}}$. We shall ignore the task of showing well-typedness here (even though that it in some places is non-trivial \todo{question}). So we assume some $\rho \in \TenvInp{\tenv}$ and $\gamma \in \VenvInp{\venv}{\rho}$, and must then show
  ...
  \end{equation}
  for some values $\val_{y_1}, \val_{y_2}$.
  \todo{question}
  We also get that $\jdg{\emptenv}{\empvenv}{\gamma_2(\expr_1)}{\rho_1(\typ)}$.
  ...
  Finally, in our goal (\ref{lamhoist:goal_2}), we use $\val_{f_1}$ for $\val_1$, and $\val_{f_2}$ for $\val_2$. The first derivation holds by
  \begin{align*}
  ...
  \begin{align*}
    \subst{(\Let \varB = \gamma_2(\expr_1) in \gamma_2(\expr_2))}{\var}{\valB_2}
    &\equiv
    \subst{(\Let \varB = \val_{y_2} in \gamma_2(\expr_2))}{\var}{\valB_2}\\
    &\step
    \subst{(\subst{\gamma_2(\expr_2)}{\varB}{\val_{y_2}})}{\var}{\valB_2}\\
    &\equiv
    \subst{\gamma_2[\map{\varB}{\val_{y_2}}](\expr_2)}{\var}{\valB_2}\\
    &\equiv
    \gamma_2[\map{\varB}{\val_{y_2}}][\map{\var}{\valB_2}](\expr_2)\\
    &\equiv
    \gamma'_2(\expr_2)\\
    &\stepS \val_{f_2}
  \end{align*}
  \todo{question end}
  \todo{annotate each line in the "correct" derivation}
  And $\ValInp{\typ_2}{\rho}(\val_{f_1}, \val_{f_2})$ follows from (\ref{lamhoist:2.1}).
\end{proof}
