...
\begin{lemma}[Evaluation under Context]
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item $\elctx = H[E[]]$ and $E[\expr] = h$.
        Here we have $h = E[\expr]$, so $E[\expr] \hstep h'$. Note that $E$ is not the empty evaluation context, as otherwise, we would be in case 1. So by \todo{create lemma and ref here}, we know that $Val(\expr)$. Now, pick $\expr$ for $\expr''$ in \ref{eqn:EuCGoal}. We must show 
        \begin{equation*}
          (\expr \stepS \expr) \land 
          \left( (Val(\expr) \land \elctx[\expr] \stepS \expr') \lor
          (\neg Val(\expr) \land \elctx[\expr] = \expr') \right)
...
% Talk about non-base types, like function type.
