...
\begin{definition}\label{def:CE2}
  Contextual Equivalence is a relation, $R \subseteq \tenv \times \tctx \times \expr \times \expr \times \typ$, such that
  \begin{itemize}
...
\end{definition}
We introduce the following notation: $R(\tenv, \tctx, \expr, \expr', \typ) \triangleq \envscteqgen \tenv \tctx \exp1 \expr \exp2 \expr' \type \typ$, and use them interchangeably.\\ 
That $ctx-eqiuv$ is a congruence relation with respect to the typing rules means that it satisfies all the congruence rules that arises from the typing rules in the same manner CONGRUENCE-FUNCTION-APP did. For instance, the congruence rule for T-unit and T-app would be:
\begin{mathpar}
  \inferrule*[lab=Congruence-unit]{ }
    { \envscteqgen \tenv \tctx \exp1 \TT \exp2 \TT \type {\Tunit} }
  \and
  \inferrule*[lab=Congruence-type-app]
    { \envscteqgen \tenv \tctx \exp1 \expr \exp2 {\expr'} \type {\Tall{\Tvar}{\typ}} }
    { \envscteqgen \tenv \tctx \exp1 \Tapp{\expr} \exp2 {\Tapp{\expr'}} \type {\subst{\typ}{\Tvar}{\typ'}} }
\end{mathpar}
The next part, $ctx-equiv$ being an adequate relation, essentially just means that it satisfies $\envscteqgen \emptctx \emptenv \exp1 \expr \exp2 \expr' \type \Tunit \implies \expr \Downarrow \iff \expr' \Downarrow$.\\
Finally, the fact that $ctx-equiv$ is the coarsest such relation simply means the following: $\forall \tenv, \tctx, \expr, \expr', \typ . R(\tenv, \tctx, \expr, \expr', \typ) \implies ctx-equiv(\tenv, \tctx, \expr, \expr', \typ)$.\\
...
\begin{theorem}\label{thm:CE_eq}
  $\envscteq \tenv \tctx \exp1 \expr \exp2 \expr' \type \typ \iff ctx-equiv(\tenv, \tctx, \expr, \expr', \typ)$
\end{theorem}
...
\begin{theorem}\label{thm:CE_eq_part1}
  $\envscteq \tenv \tctx \exp1 \expr \exp2 \expr' \type \typ \subseteq \tenv \times \tctx \times \expr \times \expr \times \typ$\text{is a congruence relation,}\\\text{ an adequate relation, and all expressions in the relation are well-typed.}
\end{theorem}
\begin{proof}
  First, the well-typedness follows directly from the definition of contextual equivalence.\\
  Second, let's show that it is an adequate relation. So assuming $\envscteq \emptctx \emptenv \exp1 \expr \exp2 \expr' \type \Tunit$, we must show $\expr \Downarrow \iff \expr' \Downarrow$. We know that $\forall \ctx : (\envstyp \emptenv \emptctx \type \Tunit) \Rightarrow (\envstyp {\emptenv} {\emptctx} \type \Tunit) . (\ctx[\expr] \Downarrow \iff \ctx[\expr'] \Downarrow)$. So specifically for $C$ being the empty context, we get $\expr \Downarrow \iff \expr' \Downarrow$, which was what we wanted.\\
...
\begin{lemma}\label{lem:CE_eq_part2_helper}
  $R(\tenv, \tctx, \expr, \expr', \typ) \; \land \; C:(\envstyp \tenv \tctx \type \typ \Rightarrow \envstyp {\tenv'} {\tctx'} \type {\typ'}) \implies R(\tenv', \tctx', C[\expr], C[\expr'], \typ')$
\end{lemma}
\begin{proof}
  \todo{do proof}
\end{proof}
...
\begin{theorem}\label{thm:CE_eq_part2}
  $R(\tenv, \tctx, \expr, \expr', \typ) \implies \envscteq \tenv \tctx \exp1 \expr \exp2 \expr' \type \typ$.
\end{theorem}
\begin{proof}
  So assuming $R(\tenv, \tctx, \expr, \expr', \typ)$, we must show 
  $ \envs \tenv \tctx \exp \expr \type \typ \; \land \;
    \envs \tenv \tctx \exp \expr' \type \typ \; \land\\
    \forall \ctx : (\envstyp \tenv \tctx \type \typ) \Rightarrow (\envstyp {\emptenv} {\emptctx} \type \Tunit) . (\ctx[\expr] \Downarrow \iff \ctx[\expr'] \Downarrow)$. Again, the well-typedness follows from the fact that all expressions in $R$ are well-typed. So what remains to be shown is that $\forall \ctx : (\envstyp \tenv \tctx \type \typ) \Rightarrow (\envstyp {\emptenv} {\emptctx} \type \Tunit) . (\ctx[\expr] \Downarrow \iff \ctx[\expr'] \Downarrow)$. So assume some context $C$ of the right type. By lemma \ref{lem:CE_eq_part2_helper}, we have $R(\emptenv, \emptctx, C[\expr], C[\expr'], \Tunit)$. By adequacy of $R$, we have that $\ctx[\expr] \Downarrow \iff \ctx[\expr'] \Downarrow$, which was what we wanted.
\end{proof}
...
\begin{proof}[Proof of theorem \ref{thm:CE_eq}]
  By theorem \ref{thm:CE_eq_part1} and from the fact that $ctx-equiv$ is the coarsest relation satisfying the three properties, it follows that $\envscteq \tenv \tctx \exp1 \expr \exp2 \expr' \type \typ \implies ctx-equiv(\tenv, \tctx, \expr, \expr', \typ)$.\\
  By theorem \ref{thm:CE_eq_part2} and from the fact that $ctx-equiv$ is one such relation $R$, it follows that $ctx-equiv(\tenv, \tctx, \expr, \expr', \typ) \implies \envscteq \tenv \tctx \exp1 \expr \exp2 \expr' \type \typ$
\end{proof}
