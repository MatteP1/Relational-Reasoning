...
\begin{mathpar}
  \inferrule*[lab=Cng-unit]{ }
    {\jdgRel{\tenv}{\venv}{\TT}{R}{\TT}{\Tunit}}
  \and
  \inferrule*[lab=Cng-Tapp]
    { \jdgRel{\tenv}{\venv}{\expr}{R}{\expr'}{\Tall{\Tvar}{\typ}} }
    { \jdgRel{\tenv}{\venv}{\Tapp{\expr}}{R}{\Tapp{\expr'}}{\subst{\typ}{\Tvar}{\typ'}} }
\end{mathpar}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    %   Then $\ctx : (\jdgType{\tenv}{\venv}{\typ} \Rightarrow \jdgType{\tenv'}{\venv'}{\Tall{\Tvar}{\typ'}})$ and $\ctx' : (\jdgType{\tenv}{\venv}{\typ} \Rightarrow \jdgType{\tenv', \Tvar}{\venv'}{\typ'})$, so our goal is $R(\tenv', \venv', \ctx[\expr], \ctx[\expr'], \Tall{\Tvar}{\typ'})$. By I.H. we get $\jdgRel{\tenv', \Tvar}{\venv'}{\ctx'[\expr]}{R}{\ctx'[\expr']}{\typ'}$, and by cng-tlam with this, we get $\jdgRel{\tenv'}{\venv'}{\Tlam \ctx'[\expr]}{R}{\Tlam \ctx'[\expr']}{\Tall{\Tvar}{\typ'}} \equiv \jdgRel{\tenv'}{\venv'}{\ctx[\expr]}{R}{\ctx[\expr']}{\Tall{\Tvar}{\typ'}}$.
  \end{itemize}
\end{proof}
...
Initially, we shall define the \textit{value interpretation} that states which values are logically related. Following this, we define the \textit{expression interpretation}, which talks about relatedness for \textit{closed} expressions. Finally, we shall define the \textit{type environment interpretation} and \textit{variable environment interpretation}, which will give us relational substitutions allowing us to close off open expressions so that we may define our complete LR model on open expressions.
...
\begin{align*}
  ...
  \right\}
\end{align*}
This set is not very restrictive. In proofs, if we have two type abstractions that are logically related, then the person proving gets to pick $R$. This gives a lot of power as we will see in chapter \ref{ch:ACE}.\\
...
\end{align*}
Second, our interpretation for variable environments is as follows.
\begin{align*}
...
As mentioned in the first section of this chapter, we want to show that LR is a congruency relation. So we must go through each congruency rule mentioned in appendix \ref{appendix:Cng} (but with the relation being LR) and show that it holds. We refer to these lemmas as \textit{compatibility lemmas}. Some compatibility lemmas are very similar, so we show only a selected few. Two additional compatibility lemmas can be found in appendix \ref{appendix:CL_extra}. For the sake of brevity, we only show well-typedness in the first two compatibility lemmas.
...
With the compatibility lemmas proved, we get that LR is a congruence relation.
...
\cleardoublepage
\appendix
\chapter{Congruence Rules for a Relation R}\label{appendix:Cng}
\begin{mathpar}
  \inferrule*[lab=Cng-var]
    {(\var : \typ) \in \venv}
    {\jdgRel{\tenv}{\venv}{\var}{R}{\var}{\typ} }
  \and
  \inferrule*[lab=Cng-unit]{ }{ \jdgRel{\tenv}{\venv}{\TT}{R}{\TT}{\Tunit} }
  \and
  \inferrule*[lab=Cng-int]{ }{ \jdgRel{\tenv}{\venv}{\Num{n}}{R}{\Num{n}}{\Tint} }
  \and
  \inferrule*[lab=Cng-add]
    { {\jdgRel{\tenv}{\venv}{\expr_1}{R}{\expr_1'}{\Tint} } \and
    { \jdgRel{\tenv}{\venv}{\expr_2}{R}{\expr_2'}{\Tint} } }
    { \jdgRel{\tenv}{\venv}{\expr_1 + \expr_2}{R}{\expr_1' + \expr_2'}{\Tint} }
  \and
  ...
  \inferrule*[lab=Cng-le]
    { {\jdgRel{\tenv}{\venv}{\expr_1}{R}{\expr_1'}{\Tint} } \and
    { \jdgRel{\tenv}{\venv}{\expr_2}{R}{\expr_2'}{\Tint} } }
    { \jdgRel{\tenv}{\venv}{\expr_1 \le \expr_2}{R}{\expr_1' \le \expr_2'}{\Tbool} }
  \and
  \inferrule*[lab=Cng-lt]
    { {\jdgRel{\tenv}{\venv}{\expr_1}{R}{\expr_1'}{\Tint} } \and
    { \jdgRel{\tenv}{\venv}{\expr_2}{R}{\expr_2'}{\Tint} } }
    { \jdgRel{\tenv}{\venv}{\expr_1 < \expr_2}{R}{\expr_1' < \expr_2'}{\Tbool} }
  \and
  \inferrule*[lab=Cng-eq]
    ...
    { \jdgRel{\tenv}{\venv}{\expr_1 = \expr_2}{R}{\expr_1' = \expr_2'}{\Tbool} }
  \and
  \inferrule*[lab=Cng-true]{ }{ \jdgRel{\tenv}{\venv}{\True}{R}{\True}{\Tbool} }
  \and
  \inferrule*[lab=Cng-false]{ }{ \jdgRel{\tenv}{\venv}{\False}{R}{\False}{\Tbool} }
  \and
  \inferrule*[lab=Cng-if]
    { {\jdgRel{\tenv}{\venv}{\expr_1}{R}{\expr_1'}{\Tbool} } \and
    { \jdgRel{\tenv}{\venv}{\expr_2}{R}{\expr_2'}{\typ} } \and
    { \jdgRel{\tenv}{\venv}{\expr_3}{R}{\expr_3'}{\typ} }
    }
    { \jdgRel{\tenv}{\venv}{\If \expr_1 then \expr_2 else \expr_3}{R}{\If {\expr_1'} then {\expr_2'} else {\expr_3'}}{\typ} }
\end{mathpar}
\begin{mathpar}
  \inferrule*[lab=Cng-pair]
    { {\jdgRel{\tenv}{\venv}{\expr_1}{R}{\expr_1'}{\typ_1} } \and
    { \jdgRel{\tenv}{\venv}{\expr_2}{R}{\expr_2'}{\typ_2} } }
    { \jdgRel{\tenv}{\venv}{(\expr_1,\expr_2)}{R}{(\expr_1', \expr_2')}{\Tprod{\typ_1}{\typ_2}} }
  \and
  \inferrule*[lab=Cng-fst]
    { \jdgRel{\tenv}{\venv}{\expr}{R}{\expr'}{\Tprod{\typ_1}{\typ_2}} }
    { \jdgRel{\tenv}{\venv}{\Fst \expr}{R}{\Fst \expr'}{\typ_1} }
  \and
  \inferrule*[lab=Cng-snd]
    { \jdgRel{\tenv}{\venv}{\expr}{R}{\expr'}{\Tprod{\typ_1}{\typ_2}} }
    { \jdgRel{\tenv}{\venv}{\Snd \expr}{R}{\Snd \expr'}{\typ_2} }
  \and
  \inferrule*[lab=Cng-inj1]
    { \jdgRel{\tenv}{\venv}{\expr}{R}{\expr'}{\typ_1} }
    { \jdgRel{\tenv}{\venv}{\Inj{1} \expr}{R}{\Inj{1} \expr'}{\Tsum{\typ_1}{\typ_2}} }
  \and
  \inferrule*[lab=Cng-inj2]
    { \jdgRel{\tenv}{\venv}{\expr}{R}{\expr'}{\typ_2} }
    { \jdgRel{\tenv}{\venv}{\Inj{2} \expr}{R}{\Inj{2} \expr'}{\Tsum{\typ_1}{\typ_2}} }
  \and
  \inferrule*[lab=Cng-match]
    { {\jdgRel{\tenv}{\venv}{\expr_1}{R}{\expr_1'}{\Tsum{\typ_1}{\typ_2}} } \and
    { \jdgRel{\tenv}{\venv, \var : \typ_1}{\expr_2}{R}{\expr_2'}{\typ} } \and
    { \jdgRel{\tenv}{\venv, \var : \typ_2}{\expr_3}{R}{\expr_3'}{\typ} }
    }
    { \jdgRel{\tenv}{\venv}{\Match \expr_1 with \Inj{1} \var => \expr_2 | \Inj{2} \var => \expr_3 end}{R}{\Match \expr_1' with \Inj{1} \var => \expr_2' | \Inj{2} \var => \expr_3' end}{\typ} }
  \and
  \inferrule*[lab=Cng-lam]
    { \jdgRel{\tenv}{\venv, x : \typ_1}{\expr}{R}{\expr'}{\typ_2} }
    { \jdgRel{\tenv}{\venv}{\fun \var . \expr}{R}{\fun \var . \expr'}{\Tfunc{\typ_1}{\typ_2}} }
  \and
  \inferrule*[lab=Cng-app]
    { {\jdgRel{\tenv}{\venv}{\expr_1}{R}{\expr_1'}{\Tfunc{\typ_1}{\typ_2}} } \and
    { \jdgRel{\tenv}{\venv}{\expr_2}{R}{\expr_2'}{\typ_1} } }
    { \jdgRel{\tenv}{\venv}{\expr_1 \; \expr_2}{R}{\expr_1' \; \expr_2'}{\typ_2} }
  \and
  \inferrule*[lab=Cng-Tlam]
    { \jdgRel{\tenv, \Tvar}{\venv}{\expr}{R}{\expr'}{\typ} }
    { \jdgRel{\tenv}{\venv}{\Tlam{\expr}}{R}{\Tlam{\expr'}}{\Tall{\Tvar}{\typ}} }
  \and
  \inferrule*[lab=Cng-Tapp]
    { \jdgRel{\tenv}{\venv}{\expr}{R}{\expr'}{\Tall{\Tvar}{\typ}} }
    { \jdgRel{\tenv}{\venv}{\Tapp{\expr}}{R}{\Tapp{\expr'}}{\subst{\typ}{\Tvar}{\typ'}} }
