...
We will define our LR model in several stages, where each stage increases the generality of the expressions we allow to be in our relation. The first stage defines which pairs of \textit{values} are in the relation. We will denote this as the "value interpretation". A property that we want to hold for all pairs in the value interpretation is that they are closed and well-typed, so we shall firstly define a well-typedness relation as follows:
\begin{equation*}
  \WtInpGen{\typ}{\rho} \triangleq \jdg{\emptenv}{\empvenv}{\val_1}{\rho_1(\typ)} \land \jdg{\emptenv}{\empvenv}{\val_2}{\rho_2(\typ)}\\
\end{equation*}
The $\rho$ part will be explained shortly. For now, think of $\rho_i(\typ)$ simply as a closed type.\\
...
\begin{align*}
  ...
  \end{aligned}
\end{align*}
For product types, the first value in each pair should be logically related, and likewise with the second value in each pair. Note also how the design principle of the property being preserved by eliminating forms comes into play for these three types – eliminating a pair, for instance, corresponds to taking out one of the two values, and in our definition, we also talk about the property for the inner values.\\
In the relation for function types, we mention another relation $\mathcal{E}$. This is the expression interpretation which we will define in a bit. Informally, two expressions are in our expression interpretation when they terminate at logically related values.\\
...
Now we move on to the next stage. Here we shall define the "expression interpretation", which capture which pairs of \textit{closed} expressions we consider to be logically related.
\begin{multline*}
...
At this point, we are only talking about closed expressions. To allow us to talk about open expressions as well, we need to define interpretations for type environments $\tenv$ and variable environments $\venv$, which will give us relational substitutions that we can use to close off open types and open expressions. Firstly, we define the interpretation for type environments inductively as follows.
\begin{align*}
...
\end{align*}
Secondly, our interpretation for a variable environment is as follows.
\begin{align*}
...
\end{align*}
Like with the relational type substitution, we also have the two projections of $\gamma$, $\gamma_1 = \{\var \mapsto \val_1, \dots\}$ and $\gamma_2 = \{\var \mapsto \val_2, \dots\}$.
...
Informally, the first theorem says that an expression $\expr$, which given a value of any type $\typ$ outputs an expression of the same type $\typ$, will always output the value it was given as input. The intuition for why this is, is that since the expression works for any type, it cannot do anything specific to the value given. Since it must output an expression of the same type, and it doesn't know in advance the type of the input, it only has one option: to return the value it was given. \todo{question: how does this relate to theorems for free?}
...
% * add recursive functions (a fixed-point combinator) to the language.
...
\chapter{Omitted proofs}\label{appendix:om_proof}
