...
We will not define formally how a substitution $\subst{\expr}{\var}{\expr'}$ works here, but refer instead to section 5.3 in \cite{DBLP:books/daglib/0005958}, which explains how to formally define substitutions so that they are \textit{capture-avoiding}. However, we will be needing a way to close off open expressions later, so we shall define here an extended substitution $\gamma = \{\map{\var_1}{\val_1}, \map{\var_2}{\val_2}, \dots\}$ inductively as follows.
\begin{align*}
  \emptyset(\expr) &= \expr\\
