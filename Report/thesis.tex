...
\begin{definition}[Adequacy]
  We say $R$ is an adequate relation when it holds for $R$ that $\jdgRel{\empvenv}{\emptenv}{\expr}{}{\expr'}{\Tbool} \implies \expr \Downarrow \val \iff \expr' \Downarrow \val$.
\end{definition}
...
\begin{multline*}
  \ExpInp{\typ}{\rho}(\expr_1, \expr_2) \triangleq
    \jdg{\emptenv}{\empvenv}{\expr_1}{\rho_1(\typ)} \land \jdg{\emptenv}{\empvenv}{\expr_2}{\rho_2(\typ)} \land \expr_1 \land\\
...
\begin{proof}
  ...
  $f' \stepS \Match \Inj{1} \valB_2 with \Inj{1} \var => \rho_2(\gamma_2(\expr'_2)) | \Inj{2} \var => \rho_2(\gamma_2(\expr'_3)) end \step \subst{\rho_2(\gamma_2(\expr'_2))}{\var}{\valB_2}$. By \ref{lem:sub} $\subst{\rho_1(\gamma_1(\expr_2))}{\var}{\valB_1} = \rho_1(\gamma_1[\map{\var}{\valB_1}](\expr_2))$.
  ...
  \end{align}
  So by \ref{cmpt-match:2.1}, we get $\rho_2(\gamma_2[\map{\var}{(\valB_1, \valB_2)}](\expr'_2)) \Downarrow$, which again, by \ref{lem:sub} gives us $\subst{\rho_2(\gamma_2(\expr'_2))}{\var}{\valB_2} \Downarrow$. Thus $f' \Downarrow$, which was what we wanted.\\
